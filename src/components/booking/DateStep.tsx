...
export function DateStep({ bookingType, startDate, endDate, onStartChange, onEndChange }: Props) {
  ...
  // Impossible range: checkout on/before check-in.
  const datesImpossible = !!startDate && !!endDate && endDate <= startDate
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          disabled={endLocked}
          className={`
            w-full border rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-50 dark:disabled:bg-slate-900 disabled:text-slate-400 dark:disabled:text-slate-600
            ${datesImpossible ? 'border-red-400 dark:border-red-500' : 'border-slate-300 dark:border-slate-600'}
          `}
        />
        {datesImpossible && (
          <p className="text-xs text-red-500 dark:text-red-400 mt-1">
            {sameDay
              ? 'Checkout must be at least one night after check-in — pick a later date.'
              : 'Checkout is before check-in. Choose a checkout date after your check-in date.'}
          </p>
        )}
      </div>
